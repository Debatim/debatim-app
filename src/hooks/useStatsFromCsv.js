...
export default function useStatsFromCsv(url) {
  ...
  const stats = useMemo(() => {
    ...
    const colInteracoes =
      headers.find((h) => norm(h).includes('interacao')) || 'Total de Interações'
    const colVisualizacoes = headers.find((h) => norm(h).includes('visualiz')) || null

    // contas únicas (ignora vazios)
    const contas = new Set(rows.map((r) => (r[colNome] ?? '').toString().trim()).filter(Boolean))
      .size
